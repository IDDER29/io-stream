...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/26 16:52:21 by idait-el          #+#    #+#             */
/*   Updated: 2025/11/26 16:52:33 by idait-el         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (1)
	{
		rd_bytes = read_and_join(fd, &stash[fd]);
		if (rd_bytes == -1)
		{
			free(stash[fd]);
			stash[fd] = NULL;
		...
		if (ft_strchr(stash[fd], '\n'))
			return (create_line_from_stash(&stash[fd]));
		if (rd_bytes == 0)
			return (handle_final_line(&stash[fd]));
	}
}
